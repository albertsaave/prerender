...
server.createPage = function(req, res) {
    ...
    if(!this.phantom) {
        console.log("CreatePage: phantom not found");
        setTimeout(function(){
            _this.createPage(req, res);
        }, 50);
    } else {
        req.prerender.phantomId = this.phantom.id;
        console.log("CreatePage: phantom found with id " + req.prerender.phantomId);
...
server.onPhantomPageCreate = function(req, res) {
    ...
    console.log("onPhantomCreate: Stage= " + req.prerender.stage + " PendingRequests=" + req.prerender.pendingRequests);
    ...
    req.prerender.page.get('settings.userAgent', function(userAgent) {
        ...
        _this._pluginEvent("onPhantomPageCreate", [_this.phantom, req, res], function() {
            ...
            req.prerender.page.open(encodeURI(req.prerender.url.replace(/%20/g, ' ')), function(status) {
              console.log("onPhantomCreate: opened page with status=" + status );
...
server.onResourceRequested = function (requestData, request) {
    console.log("onResoureceRequested: testing against requestData.url");
...
server.onResourceRequestedCallback = function (req, res, request) {
    console.log("onResourceRequestedCallback: testing against request.url");
    if (!(/google-analytics.com/gi).test(request.url) &&
        ...
        console.log("onResourceRequestedCallback: decrementing pendingRequests to " + req.prerender.pendingRequests);
    }
};
...
server.onResourceReceived = function (req, res, response) {
    ...
    if(response.id === 1) {
        ...
        console.log("onResourceReceived: response.id equals 1");
    }
    ...
    if(response.id === 1 && response.status >= 300 && response.status <= 399) {
        ...
        console.log('onResourceReceived: statusCode' + response.status );
    ...
    if ('end' === response.stage) {
        console.log("onResourceReceived: when response.stage is 'end'");
        if(response.url) {
          req.prerender.pendingRequests--;
          console.log("onResourceReceived: when response.url, exists pendingRequest=" + req.prerender.pendingRequests );
        }
        ...
        if (response.id === 1) {
            ...
            req.prerender.statusCode = response.status;
            console.log("onResourceReceived: when response.id is 1, pendingRequest=" + req.prerender.pendingRequests + " statusCode=" + req.prerender.statusCode);
        }
...
server.onResourceTimeout = function(req, res, request) {
    req.prerender.pendingRequests--;
    console.log("onResourceTimeout: decrementing pendingRequests=" + req.prerender.pendingRequests);
};
...
server.checkIfPageIsDoneLoading = function(req, res, force) {
    ...
    console.log("checkIfPageIsDoneLoading: timedOut=" + timedOut);
...
server.checkIfJavascriptTimedOut = function(req, res) {
    ...
    console.log("checkIfJavascriptTimedOut: timeout=" + timout);
...
server.evaluateJavascriptOnPage = function(req, res) {
    ...
    if(req.prerender.stage >= 2) {
      console.log("evaluateJavascriptOnPage: stage=" + req.prerender.stage);
      return;
    }
    ...
    req.prerender.page.evaluate(this.javascriptToExecuteOnPage, function(obj) {
        ...
        console.log("evaluateJavascriptOnPage: page.evaluate; lastJavascriptExecution=" + req.prerender.lastJavascriptExecution);
...
server.onPageEvaluate = function(req, res) {
    ...
    if(req.prerender.stage >= 2) {
      console.log("onPageEvaluate: stage=" + req.prerender.stage);
      return;
    }
...
server._sendResponse = function(req, res, options) {
    ...
    if (req.prerender.documentHTML) {
        ...
        console.log("sendResponse: documentHTML not empty, setting content-length");
    }

    if (!req.prerender.documentHTML) {
        res.removeHeader('Content-Length');
        console.log("sendResponse: documentHTML is empty, removing content-length");
    }
    ...
    if (req.prerender.documentHTML) {
      console.log("sendResponse: documentHTML not empty, writing documentHTML to the response");
