...
var WAIT_AFTER_LAST_REQUEST = process.env.WAIT_AFTER_LAST_REQUEST || 2500;
...
server.createPage = function(req, res) {
    ...
    if(!this.phantom) {
        ...
        console.log("CreatePage: phantom found with id " + req.prerender.phantomId + " url: " + req.prerender.url);
...
server.onPhantomPageCreate = function(req, res) {
    ...
    console.log("phantomdId:" +  req.prerender.phantomId +" " + "onPhantomCreate: Stage= " + req.prerender.stage + " PendingRequests=" + req.prerender.pendingRequests + " url: " + req.prerender.url);
    ...
    req.prerender.page.get('settings.userAgent', function(userAgent) {
        ...
        _this._pluginEvent("onPhantomPageCreate", [_this.phantom, req, res], function() {
            ...
            req.prerender.page.open(encodeURI(req.prerender.url.replace(/%20/g, ' ')), function(status) {
              console.log( "phantomdId:" + req.prerender.phantomId + " " + "onPhantomCreate: opened page with status=" + status + " url: " + req.prerender.url);
...
server.onResourceRequested = function (requestData, request) {
    console.log( "onResoureceRequested: testing against requestData.url" + requestData.url);
    if ((/google-analytics.com/gi).test(requestData.url) ||
        ...
        console.log("onResourceRequested: performing abort on request url" + requestData.url);
...
server.onResourceRequestedCallback = function (req, res, request) {
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "onResourceRequestedCallback: testing against request.url" + request.url);
    if (!(/google-analytics.com/gi).test(request.url) &&
        ...
        console.log( "phantomdId:" + req.prerender.phantomId + " " + "onResourceRequestedCallback: incrementing pendingRequests to " + req.prerender.pendingRequests);
    }
};
...
server.onResourceReceived = function (req, res, response) {
    ...
    if(response.id === 1) {
        ...
        console.log( "phantomdId:" + req.prerender.phantomId + " " + "onResourceReceived: response.id equals 1, url: " + req.prerender.url);
    }
    ...
    if(response.id === 1 && response.status >= 300 && response.status <= 399) {
        ...
        console.log( "phantomdId:" + req.prerender.phantomId + " " + 'onResourceReceived: Redirect URL:' + response.redirectURL );
        console.log(  "phantomdId:" + req.prerender.phantomId + " " + 'onResourceReceived: Redirect Response Status: ' + response.status );
        ...
        console.log( "phantomdId:" + req.prerender.phantomId + " " + 'onResourceReceived: statusCode' + response.status + " url: " + req.prerender.url);

        if(!(this.options.followRedirect || process.env.FOLLOW_REDIRECT)) {
            //force the response now
            console.log( "phantomdId:" + req.prerender.phantomId + " " + 'onResourceReceived: when folloRedirect is false or undefined ' + (this.options.followRedirect || process.env.FOLLOW_REDIRECT ));
            console.log( "phantomdId:" + req.prerender.phantomId + " " + 'onResourceReceived: returning checkIfPageIsDoneLoading() with args true');
            return this.checkIfPageIsDoneLoading(req, res, true);
        }
    }

    if ('end' === response.stage) {
        console.log( "phantomdId:" + req.prerender.phantomId + " " + "onResourceReceived: when response.stage is 'end', url: " + req.prerender.url);
        if(response.url) {
          req.prerender.pendingRequests--;
          console.log( "phantomdId:" + req.prerender.phantomId + " " + "onResourceReceived: when response.url exists, decrementing pendingRequest=" + req.prerender.pendingRequests );
        }
        ...
        if (response.id === 1) {
            ...
            req.prerender.statusCode = response.status;
            console.log( "phantomdId:" + req.prerender.phantomId + " " + "onResourceReceived: when response.id is 1, decrementing pendingRequest=" + req.prerender.pendingRequests + " statusCode=" + req.prerender.statusCode);
        }

        if( (this.options.followRedirect || process.env.FOLLOW_REDIRECT) && req.prerender.redirectURL && response.id === 1) {
            req.prerender.statusCode = response.status;
            console.log( "phantomdId:" + req.prerender.phantomId + " " + "onResourceReceived: when response.id is 1, redirectURL exists, and folloRedirect is true, statusCode=" + req.prerender.statusCode);
        }
    }
};
...
server.onResourceTimeout = function(req, res, request) {
    req.prerender.pendingRequests--;
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "onResourceTimeout: decrementing pendingRequests=" + req.prerender.pendingRequests + " url: " + req.prerender.url);
};
...
server.checkIfPageIsDoneLoading = function(req, res, force) {
    ...
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "checkIfPageIsDoneLoading: timedOut=" + timedOut + " url: " + req.prerender.url);
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "checkIfPageIsDoneLoading: timeSinceLastRequest=" + timeSinceLastRequest + " url: " + req.prerender.url);
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "checkIfPageIsDoneLoading: pendingRequests=" + req.prerender.pendingRequests + " url: " + req.prerender.url);
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "checkIfPageIsDoneLoading: force=" + force + " url: " + req.prerender.url);
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "checkIfPageIsDoneLoading: timeSinceLastRequest > WAIT_AFTER_LAST_REQUEST=" + (timeSinceLastRequest > (req.prerender.waitAfterLastRequest || this.options.waitAfterLastRequest || WAIT_AFTER_LAST_REQUEST)) + " url: " + req.prerender.url);
    if(req.prerender.stage < 1 && (force || (req.prerender.status !== null && req.prerender.pendingRequests <= 0 && (timeSinceLastRequest > (req.prerender.waitAfterLastRequest || this.options.waitAfterLastRequest || WAIT_AFTER_LAST_REQUEST))) || timedOut)) {
        ...
        if(req.prerender.status === 'fail') {
            ...
            console.log( "phantomdId:" + req.prerender.phantomId + " " + "checkIfPageIsDoneLoading: status=504");
        ...
        if(req.prerender.statusCode && req.prerender.statusCode >= 300 && req.prerender.statusCode <= 399) {
            ...
            console.log( "phantomdId:" + req.prerender.phantomId + " " + 'checkIfPageIsDoneLoading: Finish up if we got a redirect status code= ' + req.prerender.statusCode);
            res.send(req.prerender.statusCode);
        } else {
            // Now evaluate the javascript
            console.log( "phantomdId:" + req.prerender.phantomId + " " + 'checkIfPageIsDoneLoading: now evaluate the javascript, statusCode= ' + req.prerender.statusCode);
            req.prerender.downloadFinished = new Date();
            console.log( "phantomdId:" + req.prerender.phantomId + " " + 'checkIfPageIsDoneLoading: downloadFinished= ' + req.prerender.downloadFinished);
...
server.checkIfJavascriptTimedOut = function(req, res) {
    ...
    console.log( "phantomdId:" + req.prerender.phantomId + " " +  "checkIfJavascriptTimedOut: timeout=" + timeout + " url: " + req.prerender.url);
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "checkIfJavascriptTimedOut: lastJSExecutionWasLessThanTwoSecondsAgo=" +  lastJsExecutionWasLessThanTwoSecondsAgo);
    console.log( "phantomdId:" + req.prerender.phantomId + " " + "checkIfJavascriptTimedOut: noJsExecutionInFirstSecond=" +  noJsExecutionInFirstSecond);
...
server.evaluateJavascriptOnPage = function(req, res) {
    ...
    console.log("evaluateJavascriptOnPage: reached");

    if(req.prerender.stage >= 2) {
      console.log( "phantomdId:" + req.prerender.phantomId + " " + "evaluateJavascriptOnPage: stage=" + req.prerender.stage + " url: " + req.prerender.url);
      return;
    }
    ...
    req.prerender.page.evaluate(this.javascriptToExecuteOnPage, function(obj) {
        ...
        console.log("evaluateJavascriptOnPage: inside page.evaluate");
        console.log( "phantomdId:" + req.prerender.phantomId + " " + "evaluateJavascriptOnPage: page.evaluate; lastJavascriptExecution=" + req.prerender.lastJavascriptExecution + " url: " + req.prerender.url);
        console.log( "phantomdId:" + req.prerender.phantomId + " " + "evaluateJavascriptOnPage: shouldWaitForPrerenderReady=" + obj.shouldWaitForPrerenderReady);
        console.log( "phantomdId:" + req.prerender.phantomId + " " + "evaluateJavascriptOnPage: prerenderReady=" + obj.prerenderReady);
...
server.javascriptToExecuteOnPage = function() {
    try {
        ...
        if (html && html[0]) {
          console.log( 'javascriptToExecuteonPage: html NOT empty');
        ...
        console.log( 'javascriptToExecuteOnPage: Exception:');
        console.log(e);
    }

    console.log( 'javascriptToExecuteOnPage: html IS empty');

    return  {
        html: '<html><head></head><body><div> HTML is empty </div></body></html>',
...
server.onPageEvaluate = function(req, res) {
    ...
    if(req.prerender.stage >= 2) {
      console.log( "phantomdId:" + req.prerender.phantomId + " " +"onPageEvaluate: stage=" + req.prerender.stage + " url: " + req.prerender.url);
      return;
    }
    ...
    if (!req.prerender.documentHTML) {
        console.log( "phantomdId:" + req.prerender.phantomId + " " +"onPageEvaluate: when documentHTML is empty, statusCode=" + req.prerender.statusCode);
        res.send(req.prerender.statusCode || 404);
    } else {
        console.log( "phantomdId:" + req.prerender.phantomId + " " +'onPageEvalute: when documentHTML is not empty, sending page with statusCode=' + req.prerender.statusCode );
...
server._send = function(req, res, statusCode, options) {
    ...
    req.prerender.stage = 2;
    console.log( "phantomdId:" + req.prerender.phantomId + " " +"send: stage=2" + " url: " + req.prerender.url);
    ...
    console.log( "phantomdId:" + req.prerender.phantomId + " " +"send: statusCode=" + statusCode);
    ...
    this._pluginEvent("beforeSend", [req, res], function() {
        ...
        if (req.prerender.headers && req.prerender.headers.length) {
            req.prerender.headers.forEach(function(header) {
                ...
                console.log( "phantomdId:" + req.prerender.phantomId + " " +"send: header.name=" + header.name + " header.value=" + header.value );
            });
        }
        ...
        if (req.prerender.redirectURL && !(_this.options.followRedirect || process.env.FOLLOW_REDIRECT)) {
            ...
            console.log( "phantomdId:" + req.prerender.phantomId + " " +"send: redirectURL=" + req.prerender.redirectURL);
        }

        res.setHeader('Content-Type', 'text/html;charset=UTF-8');
        console.log( "phantomdId:" + req.prerender.phantomId + " " +"send: setting header ContentType to html, and UTF-8");
...
server._sendResponse = function(req, res, options) {
    ...
    if (req.prerender.documentHTML) {
        ...
        console.log( "phantomdId:" + req.prerender.phantomId + " " +"sendResponse: documentHTML not empty, setting content-length" + " url: " + req.prerender.url);
    }

    if (!req.prerender.documentHTML) {
        res.removeHeader('Content-Length');
        console.log( "phantomdId:" + req.prerender.phantomId + " " +"sendResponse: documentHTML is empty, removing content-length" + " url: " + req.prerender.url);
    }
    ...
    if (req.prerender.documentHTML) {
      console.log( "phantomdId:" + req.prerender.phantomId + " " +"sendResponse: documentHTML not empty, writing documentHTML to the response"+ " url: " + req.prerender.url );
      res.write(req.prerender.documentHTML);
    }
    res.end();
    ...
    if(options && options.abort) {
        console.log( "phantomdId:" + req.prerender.phantomId + " " +"sendResponse: calling killing phantomJS dead code ");
        this._killPhantomJS();
    }
};
